                ...
                .then((code => {
                    magister({
                        ...
                        .then((m) => {
                            ...
                            m.courses()
                                ...
                                .then(grades => {
                                    ...
                                    grades.forEach(grade => {
                                        if (sorted[grade.class.id] == null) {
                                            sorted[grade.class.id] = []
                                        }
                            ...
                            if (err.toString() == 'AuthError: Invalid password') {
                                error = `Het lijkt erop dat het wachtwoord dat je hebt gegeven niet overeenkomt met je Magister wachtwoord. Stuur je wachtwoord opnieuw met het command \`!wachtwoord\``
                            }
                            if (err.toString() == 'AuthError: Invalid username') {
                                error = `Het lijkt erop dat de gebruikersnaam die je hebt gegeven niet overeenkomt met je Magister gebruikersnaam. Stuur je gebruikersnaam opnieuw met het command \`!naam\``
                            }
                            // msg.author.send(err.toString())
                            msg.author.send(error)
                            message.edit(`Error... <@${id}> check je pm.`)
                            console.error('something went fout oof:', err);
                        });
                }))
        });
    })
